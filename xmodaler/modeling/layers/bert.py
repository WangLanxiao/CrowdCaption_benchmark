...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, history_states=None):
        mixed_query_layer = self.query(hidden_states)   # 5B * 36 * 512     5B * 20 * 512
        ...
        if history_states is not None:            
            ...
            mixed_value_layer = self.value(hidden_states)   # 5B * 36 * 512     5B * 20 * 512

        query_layer = self.transpose_for_scores(mixed_query_layer)   #  5B * 8 * 36 * 64    # 5B * 8 * 20 * 64
        key_layer = self.transpose_for_scores(mixed_key_layer)
        value_layer = self.transpose_for_scores(mixed_value_layer)

        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))   #  5B * 8 * 20 * 20
