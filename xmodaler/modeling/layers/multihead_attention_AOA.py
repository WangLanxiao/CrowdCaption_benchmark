...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None, history_states=None):
        ...
        # change xxxxxxxxxxxx aoanet
        single_query = 0
        if len(queries.shape) != len(keys.shape):
            single_query = 1
...
class AOAAttention(nn.Module):
    '''
    AoA attention layer with Dropout and Layer Normalization.
    '''
    def __init__(
            self,
            *,
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None, history_states=None):
        ...
        out, _ = self.attention(queries, keys, values, attention_mask, attention_weights, history_states)
        out = self.aoa_layer(self.dropout(torch.cat([out, queries], -1)))
        return out
