...
class BaseEncoderDecoder(nn.Module, metaclass=ABCMeta):
    ...
    def preprocess_batch(self, batched_inputs):
        ...
        if kfg.HRNET_FEATS in batched_inputs[0]:           # xxxxxxxxxxxxxxxxxxxxxxxxxxxxx   0
            hrnet_feats = [x[kfg.HRNET_FEATS] for x in batched_inputs]
            hrnet_feats = pad_tensor(hrnet_feats, padding_value=0, use_mask=False)
            ret.update({kfg.HRNET_FEATS: hrnet_feats})
        ...
        if kfg.G_OBJECT_IDS in batched_inputs[0]:   #xxxxxxxxxxxxxxxxxxx  1
            object_target_ids = [x[kfg.G_OBJECT_IDS] for x in batched_inputs]
            object_target_ids = pad_tensor(object_target_ids, padding_value=-1, use_mask=False)
            ret.update({ kfg.G_OBJECT_IDS: object_target_ids })

        if kfg.G_BEHAVIOR_IDS in batched_inputs[0]:  #xxxxxxxxxxxxxxxxxxx  2
            behavior_target_ids = [x[kfg.G_BEHAVIOR_IDS] for x in batched_inputs]
            behavior_target_ids = pad_tensor(behavior_target_ids, padding_value=-1, use_mask=False)
            ret.update({ kfg.G_BEHAVIOR_IDS: behavior_target_ids })

        if kfg.G_ATTRIBUTION_IDS in batched_inputs[0]:  #xxxxxxxxxxxxxxxxxxx  3
            attribution_target_ids = [x[kfg.G_ATTRIBUTION_IDS] for x in batched_inputs]
            attribution_target_ids = pad_tensor(attribution_target_ids, padding_value=-1, use_mask=False)
            ret.update({ kfg.G_ATTRIBUTION_IDS: attribution_target_ids })

        if kfg.G_SENTENCE_IDS in batched_inputs[0]:  #xxxxxxxxxxxxxxxxxxx  4
            sentence_target_ids = [x[kfg.G_SENTENCE_IDS] for x in batched_inputs]
            sentence_target_ids = pad_tensor(sentence_target_ids, padding_value=-1, use_mask=False)
            ret.update({ kfg.G_SENTENCE_IDS: sentence_target_ids })
        ...
        if kfg.SEQ_PER_SAMPLE in batched_inputs[0]:
            ...
            if kfg.HRNET_FEATS in batched_inputs[0]:
                d1, d2, d3 = hrnet_feats.size()[1:4]
                hrnet_feats = hrnet_feats.unsqueeze(1).expand(batch_size, repeat_num, d1, d2, d3)
                hrnet_feats = hrnet_feats.reshape(-1, d1, d2, d3)
                ret.update({kfg.HRNET_FEATS: hrnet_feats})
