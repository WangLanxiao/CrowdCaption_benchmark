...
class DefaultTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        if self.val_data_loader is not None:
            self.val_evaluator = build_evaluation(cfg, cfg.INFERENCE.VAL_ANNFILE, cfg.OUTPUT_DIR)
        else:
    ...
    def build_hooks(self):
        ...
        def test_and_save_results(epoch):
            eval_results = self.test(self.cfg, self.model, self.test_data_loader, self.test_evaluator, epoch, mode='test')
            return eval_results

        def val_and_save_results(epoch):
            eval_results = self.test(self.cfg, self.model, self.val_data_loader, self.val_evaluator, epoch, mode='val')
            return eval_results
    ...
    @classmethod
    def test(cls, cfg, model, test_data_loader, evaluator, epoch, mode = None):
        model.eval()
        results = []
        with torch.no_grad():
        ...
        if evaluator is not None:
            eval_res = evaluator.eval(results, epoch, mode)   #wlx
        else:
            eval_res = ''
        model.train()
        return eval_res
